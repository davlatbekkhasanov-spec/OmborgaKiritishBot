...
def resolve_db_path(*, env_key: str = "HUB_DAY_LOG_PATH", default_filename: str = "omborga_hub_day.db") -> str:
...
def migrate_legacy_db(target_path: str, *extra_legacy: str) -> str | None:
    ...
    for p in (
        *extra_legacy,
        base,
        "omborga_hub_day.db",
        os.path.join("/app", base),
        os.path.join("/app", "omborga_hub_day.db"),
    ):
...
def bootstrap_persistence(
    db_path: str,
    *,
    legacy_names: tuple[str, ...] = ("omborga_hub_day.db",),
) -> dict:
